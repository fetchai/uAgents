...
class EmailAssistant(Model):
    email: str = Field(description="Email of interest to the user to login to Gmail. Always request this field since it can be different from the actual user email.")
...
async def suggest_top_priority_emails(ctx: Context, sender: str, msg: EmailAssistant):
    ...
    if status == "OK":
        ...
        if list_answer == []:
            await ctx.send(
                sender, UAgentResponse(message="There was an unexpected error", type=UAgentResponseType.ERROR)
            )
        ...
        for index, answer in enumerate(list_answer):
            ...
            final_answer += f"""●{index} sender: {parsed_sender} subject: {data['subject']}\n"""
        ...
        await ctx.send(
                sender, UAgentResponse(message="Failed to retrieve emails", type=UAgentResponseType.ERROR)
            )
