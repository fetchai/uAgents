...
assert SEED_PHRASE != "-- generate data agent seed phrase--", "Please set a new seed phrase"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get('https://rpc-fetchhub.fetch.ai/blockchain') as response:
            ...
            for block in resp['result']['block_metas']:
                ...
                csv_data += ','.join([str(timestamp), str(height), block_hash, str(block_size), str(num_tx), chain_id,
                                      proposer_address]) + '\n'
...
class DataSetRequest(Model):
    timeframe: str = Field(description="The timeframe for the data. Must start with a number and be followed by either 'm' for minutes or 'h' for hours or 'd' for days or 'w' for weeks.")
