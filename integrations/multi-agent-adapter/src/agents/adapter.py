...
async def start_ai_adapter(ctx: Context):
    ...
    ctx.storage.set("ai_agents_responses", {})
...
async def send_prompt_to_ai_agents(ctx: Context, sender: str, msg: AIRequest):
    ...
    for address in ctx.storage.get("ai_agent_addresses"):
...
async def process_response_from_ai_agent(ctx: Context, sender: str, msg: AIResponse):
    ...
    # fetch AI agent responses from agent storage
    ai_agents_responses = ctx.storage.get("ai_agents_responses")
    session_str = str(ctx.session)
    if session_str in ai_agents_responses:
    ...
    ctx.logger.info(
        f"All AI Agent responses collected for all DeltaV sessions so far: {ai_agents_responses}"
    )
    # store updated AI responses data in agent storage
    ctx.storage.set("ai_agents_responses", ai_agents_responses)
    ...
    if len(ai_agents_responses_session) == len(ctx.storage.get("ai_agent_addresses")):
        ...
        ai_agents_responses.pop(session_str)
