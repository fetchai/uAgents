...
import requests
import json
from openai import get_data, send_shared_link_data, Summary, Response, Error, OPENAI_API_KEY
...
async def send_pdf_content(ctx: Context, sender :str, query: Question):
    if query.question and query.chapter and query.subject and query.standard:
        ...
        msg = Summary(summary = data["summary"], question_bank = data["question_bank"], answer_key = data["answer_key"])
