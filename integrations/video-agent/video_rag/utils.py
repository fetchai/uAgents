...
def get_batches(transcript: List[str], token_limit):
    ...
    if num_chunks <= 1:
        logger.info(f"Transcript is small enough to fit in one batch: tokens={num_tokens}")
    ...
    logger.info(f"Transcript is too big, splitting into {num_chunks} chunks, {token_limit_per_batch} tokens per batch!")
...
def get_video_script(url):
    ...
    scripts = YouTubeTranscriptApi.get_transcript(video_id, languages=("en", "en-US", "en-GB"))
