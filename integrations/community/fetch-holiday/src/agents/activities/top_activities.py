...
TOP_ACTIVITIES_SEED = os.getenv(
    "TOP_ACTIVITIES_SEED", "top_activities really secret phrase :)"
)

agent = Agent(name="top_activities", seed=TOP_ACTIVITIES_SEED)
...
async def get_top_activity(ctx: Context, sender: str, msg: TopActivities):
    ...
    preferred_activities_str = (
        f"You should only offer programs and activities related to {msg.preferred_activities}"
        if msg.preferred_activities
        else ""
    )
    _input = prompt.format(
        city=msg.city, date=msg.date, preferred_activities_str=preferred_activities_str
    )
    try:
        ...
        ctx.logger.info(
            f"Agent executed and got following result: {result}. Mapped to options: {options}"
        )
        await ctx.send(
            sender,
            UAgentResponse(
