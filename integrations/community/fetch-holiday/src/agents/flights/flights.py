...
agent = Agent(name="flights_adaptor", seed=FLIGHTS_SEED)
...
async def flight_offers(ctx: Context, sender: str, msg: Flights):
    ...
    try:
        response = requests.request(
            "POST", SKY_SCANNER_URL, json=payload, headers=headers
        )
        if response.status_code != 200:
            print("SKYSCANNER STATUS CODE not 200: ", response.json())
            await ctx.send(
                sender,
                UAgentResponse(message=response.text, type=UAgentResponseType.ERROR),
            )
            return
