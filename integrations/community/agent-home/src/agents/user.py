...
async def interval(ctx: Context):
    ...
    if completed:
        ...
        sub_tasks: str = plan_requests(
            inp
        )  # returns a dictionary of sub-tasks with protocol as key and sub-task as value
        ...
        for protocol, sub_task in sub_tasks.items():
            msg = await create_message(
                sub_task, request_keys[protocol], request_description, protocol
            )
            print("msg", msg)
