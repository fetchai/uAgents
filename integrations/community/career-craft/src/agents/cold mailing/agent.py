...
async def handle_message(ctx: Context, sender: str, msg: Request):
    response = await send_mail_request(
        msg.job_description, msg.mail_type, msg.recruiterEmail, msg.name
    )
    await ctx.send(
        sender,
        UAgentResponse(
            message="Email Sent Successfully", type=UAgentResponseType.FINAL
        ),
    )
