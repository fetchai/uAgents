...
@generate_article_summary_protocol.on_message(
    model=GenerateArticleSummary, replies=UAgentResponse
)
async def on_generate_article_summary_request(
    ctx: Context, sender: str, msg: GenerateArticleSummary
):
    ctx.logger.info(
        f"Received article summary request from {sender} with URL: {msg.url}"
    )
    try:
