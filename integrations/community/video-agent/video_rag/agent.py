...
class SummaryRequest(Model):
    shortened: bool = Field(
        default=False,
        description="If true, the summary will be short! SET this to true if user is asking for short summary/article! If user doesn't say they want a short answer, set it to false. It's important if user is asking for a short answer set this to true.",
    )
    model: str = Field(
        default="gpt-4",
        description="The model to be used for summarization! Available models: gpt-4, gpt-3.5, claude-3. Set this to gpt-4 for best results! If the user wants fast results, set this to gpt-3.5! If they specify a model, use that (if available)!",
    )
    video_link: str = Field(description="The link to the video to be summarized!")
    question: str = Field(
        description="If the user asks for short summary, or summary set this field to empty. If the user asks to answer a question based on the video, set this field to the question!"
    )
...
async def handle_message(ctx: Context, sender: str, msg: SummaryRequest):
    if msg.model not in ["gpt-4", "gpt-3.5", "claude-3"]:
        await ctx.send(
            sender,
            UAgentResponse(
                message="Invalid model! Following models are available: gpt-4, gpt-3.5, claude-3",
    ...
    if len(msg.question) == 0:
        if msg.shortened == True:
            ...
            await ctx.send(
                sender,
                UAgentResponse(
                    message=f"Article about the video: {article}",
                    type=UAgentResponseType.FINAL,
                ),
            )
        return
