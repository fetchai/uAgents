    ...
    audio = client.generate(
        ...
        voice=Voice(
            ...
            settings=VoiceSettings(
                stability=1, similarity_boost=1, style=1, use_speaker_boost=True
            ),
        ),
    )
    ...
    try:
        ...
        for reel in reels:
            ...
            for scene in reel["scenes"]:
                fileName = await createImageFromText(
                    scene["background_image"],
                    scene["text_overlay"],
                    "{reelId}-{serial_of_image}",
                )
        ...
        await ctx.send(
            sender,
            UAgentResponse(
                message="Hello we have created a few reels from your website for you. Please have a look at them.\n\n"
                + urls,
