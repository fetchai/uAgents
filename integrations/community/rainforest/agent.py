...
class SearchAmazonRequest(Model):
    search_term: str = Field(
        description="Describes the text field to query for a product to search in marketplace"
    )
...
async def search_amazon_q(ctx: Context, sender: str, msg: SearchAmazonRequest):
    ctx.logger.info(
        f"Received message from {sender} to search Amazon for '{msg.search_term}'"
    )
    try:
