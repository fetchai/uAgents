...
async def get_flight_details_by_number(flight_number):
    ...
    search_querystring = {
        "query": flight_number,
        "limit": "1",
    }  # Assuming we're interested in the most relevant result
    headers = {
    ...
    search_response = requests.get(
        search_url, headers=headers, params=search_querystring
    )
    if search_response.status_code == 200 and search_response.json():
        ...
        detail_response = requests.get(
            detail_url, headers=headers, params=detail_querystring
        )
        if detail_response.status_code == 200:
            flight_details = detail_response.json()
            if (
                "airport" in flight_details
                and "origin" in flight_details["airport"]
                and "destination" in flight_details["airport"]
            ):
                ...
                scheduled_departure = await unix_to_readable(
                    flight_details["time"]["scheduled"]["departure"]
                )
                scheduled_arrival = await unix_to_readable(
                    flight_details["time"]["scheduled"]["arrival"]
                )
                estimated_arrival = (
                    await unix_to_readable(
                        flight_details["time"]["estimated"]["arrival"]
                    )
                    if flight_details["time"].get("estimated", {}).get("arrival")
                    else "Not available"
                )
                ...
                geo_querystring = {
                    "latitude": str(last_point["lat"]),
                    "longitude": str(last_point["lng"]),
                    "range": "0",
                }

                # Making a request to the google-maps-geocode API
                geo_response = requests.get(
                    geo_url, headers=headers1, params=geo_querystring
                )
            else:
