...
class UAgentRegisterToolInput(BaseModel):
    ...
    mailbox: bool = Field(True, description="Whether to use mailbox (True) or endpoint (False) for agent configuration")
...
class UAgentRegisterTool(BaseTool):
    ...
    def _langchain_to_uagent(self, agent_obj, agent_name, port, description=None, ai_agent_address=None, mailbox=True):
        ...
        async def startup(ctx: Context):
            ...
            ctx.logger.info(f"Agent '{agent_name}' started with address: {agent_address}")
            ...
            try:
                ...
                for item in msg.content:
                    if isinstance(item, StartSessionContent):
                        ...
                        if not ai_agent_address:
                            ctx.logger.warning("No AI agent address configured, processing directly")
                            ...
                            continue
                        ctx.logger.info(f"Sending structured output prompt to {QueryMessage.schema()}")
                        await ctx.send(
                            ai_agent_address,
        ...
        async def handle_structured_output_response(
            ...
            try:
                ...
                ctx.logger.info(f"Received structured output response from {sender}: {query.query}")
    ...
    def _register_agent_with_agentverse(self, agent_info):
        ...
        try:
            ...
            if not agent_info.get("mailbox", True):
                print(f"Agent '{agent_info.get('name')}' using endpoint configuration, skipping Agentverse registration")
                return
...
"""
    ...
    def _run(
        ...
        *,
        ...
        # Create the uAgent
        agent_info = self._langchain_to_uagent(agent_obj, name, port, description, ai_agent_address, mailbox)
