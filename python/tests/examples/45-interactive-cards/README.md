...
| Agent | Role |
|-------|------|
| `menu-host` | Receives `"menu"`, replies with a carousel card |
| `picker` | Asks for the menu, auto-selects the first carousel item, sends a card response |

Shared handlers live in `protocols.py`.
...
Watch the logs: the host prints the picker's `selection` dict after the card response.

## Mailbox agents (Agentverse)
...
sends a structured card response.
