...
@click.option('--bridge-port', 'bridge_port', default=None, help='Port for the internal uAgent bridge (auto-derived if not specified)')
@click.option('--bridge-seed', 'bridge_seed', default=None, help='Seed for bridge agent (overrides UAGENTS_BRIDGE_SEED env var)')
def main(host, port, agent_address, agent_name, agent_description, skill_tags, skill_examples, bridge_port, bridge_seed):
    ...
    try:
        ...
        if bridge_seed:
            ...
            logger.warning("⚠️  No bridge seed provided - address will be random on each restart")
            logger.warning("⚠️  Consider setting: export UAGENTS_BRIDGE_SEED='your_seed'")
        ...
        async def get_bridge_info(request):
            """Endpoint to expose bridge agent address and inspector link for frontend integration."""
            try:
                # Construct the inspector link
                bridge_address = bridge_executor.bridge_agent.address
                encoded_uri = urllib.parse.quote(f"http://127.0.0.1:{bridge_port}", safe='')
                ...
                return JSONResponse(
                    {"error": f"Failed to get bridge info: {str(e)}"},
                    status_code=500
                )
